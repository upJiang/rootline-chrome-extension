...
这套材料对应当前 `0.1.0` 构建，产品定位是：开发者在当前网页复现问题时，主动采集运行态证据，并将报告保存到本机 Chrome 下载目录，或直接上传到用户自己配置的腾讯云 COS。
...
Rootline 不提供开发者云端服务。默认本地模式不上传；用户主动选择远程模式时，报告和录屏只直接上传到用户配置的腾讯云 COS，不会上传到开发者服务器。
