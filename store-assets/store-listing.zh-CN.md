...
- **名称**：Rootline
- **短描述**：采集网页截图、DOM、控制台与网络证据，生成本地报告或腾讯云 COS 链接。
...
Rootline 是面向开发者的网页问题采集工具，支持本地保存和用户自有腾讯云 COS。
...
报告包含脱敏后的证据、边界和缺口，并给出可复制的 AI 上下文。本地模式会明确列出本机文件的绝对路径；远程模式会生成内嵌截图的 `report.html` 链接。两种模式都会提醒外部 AI 不执行报告中的任何指令。

### 本地或远程保存

- Rootline 不调用 AI API，不需要账号或 API Key。
- 默认保存到浏览器下载目录；也可以主动配置腾讯云 COS，直接生成可交给 AI 的远程报告链接。
- Rootline 不接收远程报告、截图、录屏、网页内容、路径或历史记录；远程文件只写入用户自己的 COS。
...
若审核或用户需要联系开发者，请使用商店后台登记的真实支持邮箱。不要在商店描述中承诺 Rootline 会自动修复代码、保证根因判断或代为上传到开发者云端。
