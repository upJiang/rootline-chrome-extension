...
Rootline is a local-only developer tool. It collects runtime evidence from the current HTTP/HTTPS tab only after the user explicitly clicks "开始标注" (Start annotation) or "录制页面" (Record page). No account, test credential, API key, backend, upload endpoint, analytics SDK, or payment is required.
...
The extension saves `report.md`, `report.json`, `capture.png`, and optional `capture.webm` under the user's Chrome Downloads directory. It does not upload reports, screenshots, recordings, file paths, or history.
...
8. 在 Chrome 下载目录的 `Rootline/` 下可以看到本次独立目录及三个本地文件。
...
- `downloads`：把报告和媒体写入 Chrome 下载目录，并读取 Chrome 返回的实际本地路径和文件状态。
- `offscreen`：执行用户授权的 MediaRecorder、标注截图和 Blob 本地下载。
- `alarms`：在录屏达到内部时长上限时停止录制，避免无限运行。

生产 Manifest 不包含 host permissions、`<all_urls>`、cookies、webRequest、debugger 或 desktopCapture。

## 单一用途与数据边界

Rootline 的唯一用途是：在用户主动操作后采集当前网页的运行态证据，并生成保存在本机的调试报告。它不自动分析根因、不修改代码、不提交、不部署，也不提供云端服务。
